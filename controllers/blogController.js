      ...
      const statistics = await Review.aggregate([
        {
          $group: {
            _id: "$blogId",
            numberOfReviews: { $sum: 1 }, // Count the number of reviews for each blog
            overallRating: { $avg: "$rating" } // Calculate the average rating for each blog
          }
        },
        {
          $lookup: {
            from: "blogs", // Assuming "blogs" is the name of the Blog collection
            localField: "_id",
            foreignField: "_id",
            as: "blog"
          }
